...
Using that $P(t)$ is stochastic and satisfies the semigroup propositionerty,
\[
...
\begin{proof}
(1) We use the notation $P_x(\cdot) = P(\cdot \mid X_0 = x)$. First we show the memoryless propositionerty:
\[
...
\]
Conditioning on $\{X_u = x\ \forall u \in [0,t]\}$ and applying the Markov propositionerty gives
\[
P_x(T_x > s) P_x(T_x > t),
\]
